...
WifiClient::WifiClient() : state_(STOPPED), netif_(NULL) {
  ...
  try {
    ...
    ERR_THROW(esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID,
                                         WifiClient::handle_event, this));
    ERR_THROW(esp_event_handler_register(IP_EVENT, ESP_EVENT_ANY_ID,
                                         WifiClient::handle_event, this));
}
...
void WifiClient::handle_csi(void* ctx, wifi_csi_info_t* data) {
  ESP_LOGI(TAG, "sig: %d rssi: %d rate: %d", data->rx_ctrl.sig_mode,
           data->rx_ctrl.rssi, data->rx_ctrl.rate);
}
...
void WifiClient::handle_event(void* arg, esp_event_base_t base, int32_t id,
                              void* data) {
...
void WifiClient::state_stopped(void* data, int32_t id) {
  switch (id) {
    case WIFI_EVENT_STA_START:
      ESP_LOGI(TAG, "State transition: %s -> %s on %d", "STOPPED",
               "DISCONNECTED", id);
      state_ = DISCONNECTED;
      connect();
      break;
...
void WifiClient::state_disconnected(void* data, int32_t id) {
  switch (id) {
    case WIFI_EVENT_STA_CONNECTED:
      ESP_LOGI(TAG, "State transition: %s -> %s on %d", "DISCONNECTED",
               "CONNECTED", id);
      state_ = CONNECTED;
      break;
...
void WifiClient::state_connected(void* data, int32_t id) {
  switch (id) {
    case IP_EVENT_STA_GOT_IP: {
      ...
      ESP_LOGI(TAG, "State transition: %s -> %s on %d", "CONNECTED", "ACTIVE",
               id);
      state_ = ACTIVE;
      netinfo_ = d->ip_info;
      break;
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
      ESP_LOGE(TAG, "Wifi disconnected: %s %d",
               reinterpret_cast<wifi_event_sta_disconnected_t*>(data)->ssid,
               reinterpret_cast<wifi_event_sta_disconnected_t*>(data)->reason);
      ESP_LOGI(TAG, "State transition: %s -> %s on %d", "CONNECTED",
               "DISCONNECTED", id);
      state_ = DISCONNECTED;
      connect_timer_start();
      break;
  ...
  switch (id) {
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
      ESP_LOGE(TAG, "Wifi disconnected: %s %d",
               reinterpret_cast<wifi_event_sta_disconnected_t*>(data)->ssid,
               reinterpret_cast<wifi_event_sta_disconnected_t*>(data)->reason);
      ESP_LOGI(TAG, "State transition: %s -> %s on %d", "ACTIVE",
               "DISCONNECTED", id);
      state_ = DISCONNECTED;
      connect_timer_start();
      break;
